...
export class UIManager {
    ...
    renderPlayerStatus() {
        ...
        document.getElementById('player-card-count').innerText = `카드 ${this.gameState.hands[0].length}장`;
    }
    ...
    renderAIStatus() {
        for(let i = 1; i <= 3; i++) {
            ...
            document.getElementById(`ai-${i}-cards`).innerText = `${this.gameState.hands[i].length}장`;
        }
    }
    ...
    renderCards() {
        ...
        this.gameState.hands[0].forEach((card) => {
            const isDisabled = this.gameState.turn !== 0 || 
            ...
            let icon = card.type === 'forward' ? '▲' : 
                      (card.type === 'backward' ? '▼' : 
                      (card.type === 'swap' ? '🔄' : 
                      (card.type === 'rider_fall_off' ? '💥' : '🏇')));
            
            let color = card.type === 'forward' ? 'text-blue-600' : 
                       (card.type === 'backward' ? 'text-red-600' : 
                       (card.type === 'rider_fall_off' ? 'text-orange-600' : 'text-purple-600'));
            
            let description = card.target ? `말 #${card.target}` : 
                            (card.type === 'rider_fall_off' ? '3등 → 7등' : '전략적 선택');
    ...
    renderTokenModal() {
        const canTakeToken = this.gameState.turn === 0 && 
                           this.gameState.turnPhase === 'token' && 
        ...
        const showTokenPhase = this.gameState.turn === 0 && 
                              this.gameState.turnPhase === 'token' && 
                              !this.gameState.isGameOver;
    ...
    showMessage(text, isPlayer = true) {
        ...
        setTimeout(() => { this.messageBox.style.opacity = '0'; }, 1000);
    }
